def run_whatsapp_analysis():
    ...
    import functions
    import sqlite3
    from whatsapp_analysis_storage import store_analysis_results, get_user_analyses
    import os
    import io
    ...
    # file upload
    uploaded_file = st.file_uploader("Choose a WhatsApp chat text file:")
    if uploaded_file is not None:
        ...
        selected_user = st.multiselect('Select Username', user_list)
        ...
        if st.button("Show Analysis"):
            ...
            # Store analysis results if user is logged in
            if "user_id" in st.session_state:
                analysis_stats = {
                    'total_messages': noOfmsgs,
                    'total_users': noOfUsers,
                    'total_words': noOfWords,
                    'total_media': noOfMedia,
                    'total_links': noOfLinks
                }
                store_analysis_results(conn, st.session_state["user_id"], df, analysis_stats)
            ...
            visualizations['User Activity'] = 'user_activity.png'
            
            # Save time analysis plot
            plt.figure(figsize=(12, 6))
            df['date'].value_counts().sort_index().plot()
            plt.title("Messages Over Time")
            plt.tight_layout()
            plt.savefig('time_analysis.png')
            visualizations['Time Analysis'] = 'time_analysis.png'
            ...
            # Save emoji distribution
            emoji_df = functions.get_emojis(df)
            plt.figure(figsize=(10, 8))
            emoji_df.head(10).plot(kind='pie', y='counts')
            plt.title("Top 10 Emojis")
            plt.tight_layout()
            plt.savefig('emoji_dist.png')
            visualizations['Emoji Distribution'] = 'emoji_dist.png'
            
            # Save daily activity heatmap
            plt.figure(figsize=(12, 8))
            daily_activity = pd.crosstab(df['hour'], df['day_name'])
            plt.imshow(daily_activity)
            plt.title("Daily Activity Heatmap")
            plt.colorbar()
            plt.tight_layout()
            plt.savefig('daily_heatmap.png')
            visualizations['Daily Activity'] = 'daily_heatmap.png'
            
            # Save monthly activity
            plt.figure(figsize=(10, 6))
            monthly_msgs = df['month_name'].value_counts().sort_index()
            monthly_msgs.plot(kind='bar')
            plt.title("Monthly Message Distribution")
            plt.tight_layout()
            plt.savefig('monthly_activity.png')
            visualizations['Monthly Activity'] = 'monthly_activity.png'
            ...
            stats = {
                ...
                'user_stats': {
                    'most_active_users': df['user'].value_counts().head().to_dict(),
                    'least_active_users': df['user'].value_counts().tail().to_dict(),
                },
                'time_stats': {
                    'busiest_month': df['month_name'].value_counts().index[0],
                    'busiest_day': df['day_name'].value_counts().index[0],
                    'busiest_hour': df['hour'].value_counts().index[0],
                },
                'content_stats': {
                    'top_words': word_df.sort_values(by='count', ascending=False).head(20).to_dict('records'),
                    'top_emojis': emoji_df.head(10).to_dict('records'),
                },
                'message_stats': {
                    'avg_message_length': df['message_chars'].mean(),
                    'max_message_length': df['message_chars'].max(),
                    'total_characters': df['message_chars'].sum(),
                }
            }
            ...
            pdf_path = generate_pdf_report(df, stats, visualizations)
            
            with open(pdf_path, "rb") as pdf_file:
                st.download_button(
                    label="Download Complete Analysis Report",
                    data=pdf_file,
                    file_name="whatsapp_analysis_report.pdf",
                    mime="application/pdf"
                )
            
            # Cleanup temporary files
            for vis_path in visualizations.values():
