...
def get_msg_stats(df):
    ...
    # 2. count words
    words = []
    for msg in df['message'].apply(lambda x: x.lower()):
        ms = re.split(r'[`\-=~!@#$%^&*()_+\[\]{};\'\\:"|<,./<>? ]', msg)
        for i in ms:
    ...
    missed_call = 0
    for i in df['message'].apply(lambda x: x.lower()):
        if 'missed' in i and 'call' in i:
            missed_call += 1

    return len(n_users), df.shape[0], len(words), media_count, len(links),missed_call
...
def wordMap_without_stopwords(df):
    ...
    wc = WordCloud(width=1200, height=500, min_font_size=10, max_words=75,background_color='white', regexp=r"\w[\w']+")
